...
def _bytelist2long(lst):
    ...
    while i < imax:
        b0 = int(ord(lst[j]))
        b1 = (int(ord(lst[j+1]))) << 8
        b2 = (int(ord(lst[j+2]))) << 16
        b3 = (int(ord(lst[j+3]))) << 24
        hl[i] = b0 | b1 |b2 | b3
        i = i+1
        j = j+4
...
class MD5:
    ...
    def update(self, inBuf):
        ...
        if leninBuf >= partLen:
            self.input[idx:] = [ bytes([b]) for b in inBuf[:partLen] ]
...
def generate_cram_hash(password):
    ...
    if len(password) > 64:
        p = hashlib.md5(password.encode('utf-8')).digest()
    else:
        p = password.encode('utf-8')

    password = [bytes([b]) for b in p]
    i = 0
    for b in password:
        ipad[i] = ipad[i] ^ ord(b)
        opad[i] = opad[i] ^ ord(b)
